...
class VisualEncoder(nn.Module):
    ...
    def forward(self, videos):
        ...
        # videos: batch_size * max_frame * 2048
        # mean_original: batch_size * 2048
        # mask: batch_size * max_frame
        ...
        if self.concate == 'full':  # level 1+2+3
            ...
            if self.level == '1+2':
                ...
                features = torch.cat((gru_out, con_out, org_out, output), 1)
...
class TextTransformersEncoder(nn.Module):
    ...
    def forward(self, text):
        ...
        outputs = self.model(input_ids=tokens, token_type_ids=type_ids,
                             attention_mask=mask)
        # Tensor (batch_size: 8, sequence_length: 154, hidden_size: 768)
        last_hidden = outputs[0]
        # Tensor (batch_size: 8, hidden_size: 768)
        ...
        # 局部特征
        # Tensor (batch_size: 8, 1, sequence_length: 154, hidden_size: 768)
        con_out = last_hidden.unsqueeze(1)
        # List (3 * Tensor (batch_size: 8, ch: 512, sequence_length: 154: 768))
        con_out = [F.relu(conv(con_out)).squeeze(3) for conv in self.convs1]
        # List (3 * Tensor (batch_size: 8, ch: 512))
        con_out = [F.max_pool1d(i, i.size(2)).squeeze(2) for i in con_out]
        con_out = torch.cat(con_out, 1)
        # Tensor (batch_size: 8, ch: 512 * 3)
        con_out = self.dropout(con_out)
        ...
        if self.concate == 'full':
            ...
            if self.level == '1+2':
                features = torch.cat((org_out, tf_out), 1)
            elif self.level == '1+3':
                features = torch.cat((org_out, con_out), 1)
            elif self.level == '2+3':
...
class BrandAspects(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.aspects_embeddings = nn.Parameter(
            torch.randn(self.num_aspects, self.common_embedding_size), requires_grad=True)
...
# FancyRec模型结构定义
class FancyRec(nn.Module):

    def __init__(self, opt):
        super(FancyRec, self).__init__()
        self.opt = opt
    ...
    def embed_vis(self, vis_data, volatile=True):
        # frames: batch_size * max_frame * 2048
        # mean_original: batch_size * 2048
        # vidoes_mask: mask
