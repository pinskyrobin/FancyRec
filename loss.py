...
#         # # similarity matrix
#         # scores = torch.empty((brand_emb.shape[0], brand_emb.shape[0]))
...
class TripletLoss(nn.Module):
    ...
    def forward(self, brand_ids, brand_emb, post_emb):
        ...
        # cost_b = rank_image * cost_b
        # cost_p = rank_text * cost_p
        #
        # # EET loss
        # M = scores.size(0)
        # # 约束同一个品牌到不同negative样本尽可能等距 e.g d(A,p_n1) = d(A,p_n2)
        # loss_ec1 = torch.tensor(0.0).to(device)
        # # 最大化类间距离
        ...
        #             value.append(scores[i][j])
        #             total.append(scores[i][j])
        #     tmp = torch.Tensor(value).to(device)
        #     loss_ec1 += tmp.var()
        #     loss_m += torch.sum(torch.exp(tmp))
        # loss_ec1 /= M
        # loss_m /= (M*(M-1))
