...
def get_verticals():
    ...
    :return:
    """
    data_file_path = "/root/VisualSearch/subbrand_dataset/label.csv"
...
def get_datas_from_each_vertical():
    ...
    :return:
    """
    target_root_path = '/root/VisualSearch/'
    source_root_path = '/root/VisualSearch/subbrand_dataset'
    verticals = read_dict("verticals.txt")
    vertical_list = [key for key in verticals.keys()]
    # print(verticals.keys())
    # print(vertical_list)
    for index in range(2, len(vertical_list)):
        vertical = vertical_list[index]
        ...
        # # 帧数据存放目录
        frames_save_path = os.path.join(target_root_path, vertical, "frames")
        # 切帧
        video2frame(source_root_path, videos_path, frames_save_path)
        # 视频与id的映射
        video2idx_and_idx2video(source_root_path, videos_path, vertical)
        # 将无效帧剔除
        ...
        """
        # 特征保存目录
        frame_feat_save_path = os.path.join(target_root_path, vertical)
        # 特征保存格式txt
        ...
        # # 图像特征保存的目录
        img_feat_save_path = os.path.join(target_root_path, vertical)
        ...
        process(feat_dim, input_text_file, bin_feat_dir, over_write)
        # 获取图像名到id的映射
        img2idx_and_idx2img(source_root_path, images_path, vertical)
        # ####################################################################################
        """
        六、获取视频和图像的captions
        """
        extract_video_captions(source_root_path, videos_path, vertical)
        extract_image_captions(source_root_path, images_path, vertical)
        # ####################################################################################
        """
        ...
        """
        # 划分视频和相应captions
        videos_split_train_val_test(source_root_path, target_root_path, vertical, videos_path)
        # 划分图像和相应的captions
        imgs_split_train_val_test(source_root_path, target_root_path, vertical, images_path)
        # 视觉模态、文本模态相应数据合并
        merge_captions_in_videos_and_imgs(target_root_path, vertical)
        # ####################################################################################
        """
        八、生成数据集的词文件 (用于one-hot)
        """
        commandStr = ''.join(open('template_do_get_vertical_vocab.sh', 'r', encoding='utf8').readlines())
        commandStr = commandStr.replace("@@@vertical@@@", vertical)
        commandStr = commandStr.replace("@@@collection@@@", vertical+'train')
        print(commandStr)
