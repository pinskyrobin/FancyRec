...
def process(feat_dim, inputTextFiles, result_dir, overwrite):
    """
    :param feat_dim: 每帧图像特征的维度/每张图像的特征维度
    :param inputTextFiles: 存放特征的文件或目录
    :param result_dir:  保存二进制格式特征的位置
    :param overwrite:  是否覆盖二进制特征文件
    :return:
    """
    res_binary_file = os.path.join(result_dir, 'feature.bin')
    res_id_file = os.path.join(result_dir, 'id.txt')
    ...
    for filename in inputTextFiles:
        ...
        for line in lines:
            ...
            if feat_dim == 0:
                ...
                assert (len(vec) == feat_dim), "dimensionality mismatch: required %d, input %d, id=%s, inputfile=%s" % (
                feat_dim, len(vec), name, filename)
            vec.tofile(fw)
            # print name, vec
            imset.append(name)
    fw.close()
    ...
    # fw.write(' '.join(imset))
    fw.close()
    fw = open(os.path.join(result_dir, 'shape.txt'), 'w')
...
def main(argv=None):
    ...
    if int(args[2]) == 1:
        # 依次读每个目录里的特征
        inputTextFiles = [x.strip() for x in open(inputTextFile).readlines() if
